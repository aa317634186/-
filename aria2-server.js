...
function addOptions(id) {
  const options = { dir: taskDir(id), "bt-sequential-download": "true", "bt-prioritize-piece": "head", "bt-max-peers": String(process.env.MAX_PEERS || 200), "max-upload-limit": `${process.env.UPLOAD_LIMIT_KBPS || 32}K`, "seed-time": "0", "enable-dht": "true", "bt-enable-lpd": "true", "enable-peer-exchange": "true", "bt-tracker-connect": "true", "bt-tracker-connect-timeout": "10", "bt-tracker-timeout": "10", "bt-tracker-interval": "30" };
  const trackers = process.env.BT_TRACKERS?.trim();
